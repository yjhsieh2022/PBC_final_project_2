            ...
            with open(file='C:\\Users\\Hsiao Wan-Ju\\Desktop\\TaiwanGo\\templates\\web_page2_2.html', mode='a', encoding='utf-8') as html_2:
                if API_Result[route_num][tran_num][0] == 'BUS': # BUS的API輸出順序跟其他車種不一樣，對應位置要變
                    for line in Change_22_Lines:
                        if len(API_Result[route_num][tran_num][1].strip('L')) == 4:
                            ...
                            line = line.replace("transtype", "公車"+str(API_Result[route_num][tran_num][1]))
                            line = line.replace("transprice", "市內公車、捷運票價依各縣市不同")
                        ...
                        html_2.write(line)
                elif API_Result[route_num][tran_num][0] == 'SUBWAY':
                    for line in Change_22_Lines:
                        line = line.replace("startstation", str(API_Result[route_num][tran_num][2]))
                        line = line.replace("endstation", str(API_Result[route_num][tran_num][3]))
                        line = line.replace("starttime", str(API_Result[route_num][tran_num][4]))
                        line = line.replace("transtype", str(API_Result[route_num][tran_num][1]))
                        line = line.replace("endtime", str(API_Result[route_num][tran_num][5]))
                        line = line.replace("transprice", "市內公車、捷運票價依各縣市不同")
                        html_2.write(line)
                else:
                    for line in Change_22_Lines:
